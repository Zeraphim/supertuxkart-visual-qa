...
class CLIP(nn.Module):
    def __init__(
        ...
        vision_dim = getattr(getattr(vision_encoder, "config", None), "hidden_size", None)
        if vision_dim is None:
            vision_dim = getattr(getattr(vision_encoder, "config", None), "vision_config", None).hidden_size
        text_dim = getattr(getattr(text_encoder, "config", None), "hidden_size", vision_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        return vision_feature, text_feature, logit_scale
...
def compute_clip_loss(
    ...
    logits_per_image = logit_scale * vision_feature @ text_feature.T
...
def train(
    ...
    # load dataset
    clip_processor = get_processor()
    train_dataset = CaptionDataset("train", data_dir)
    train_dataset = CaptionDatasetForTraining(train_dataset, clip_processor)
...
def test(ckpt_path: str, val_dataset: str = "valid_grader"):
    ...
    for pair in tqdm.tqdm(testset):
        ...
        text_inputs = clip_processor(
            text=[s + clip_processor.tokenizer.eos_token for s in pair["candidates"]],
            return_tensors="pt",
            padding=True,
            truncation=True,
        )
